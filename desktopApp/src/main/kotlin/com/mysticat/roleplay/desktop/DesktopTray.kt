 ...
 * 没开托盘的用户的对应手段是**启动时静默检查**（已做，见 [startupSilentCheck]——
...
internal object DesktopTray {
    ...
    /** staging 完成后待执行的替换脚本（与「关于鲸鱼」页共用 [DesktopUpdatePrompt] 这份状态） */
    ...
    internal fun applySilentCheckResult(r: Result<UpdateInfo?>, currentCode: Int): UpdateInfo? {
        ...
        latest = info
        // 应用内提醒：不依赖托盘图标，对所有人生效（窗口壳里那个 UpdateFlowHost 会弹**提醒弹窗**，
        // 同时第一栏亮起更新图标）。状态只有一份（UpdateFlow），手机端入口行那枚小字读的也是它。
        DesktopUpdateNotice.show(info)
        // 开了托盘的用户多一条自助路径（气泡 + 菜单里的「下载并安装」）；图标不在时 balloon 自己不做事
        balloon("发现新版本 ${info.versionName}", "点窗口里的「查看更新」可直接下载安装。")
        return info
    }
    ...
    private fun applyDownloadResult(info: UpdateInfo, r: Result<Pair<File, Boolean>>) {
        ...
        r.fold(
            onSuccess = { (file, isInstaller) ->
                ...
                if (isInstaller) {
                    // 安装器形态已经拉起系统向导，剩下的是用户点向导（口径同「关于鲸鱼」页）
     ...
     * [eventAnchor] 是托盘事件带来的坐标，**只当兜底**用——见 [menuLocation] 对坐标系的说明。
     */
    private fun showTrayMenu(eventAnchor: Point) {
        hideTrayMenu()
        val win = JWindow().apply {
            ...
            val column = JPanel().apply {
                ...
                add(JSeparator())
                // 更新入口挂在这里。这一行走的是纯 AWT/OkHttp，不经过任何 Compose 页面——
                // 「关于鲸鱼」那页在打包图标裁剪出错的版本里会 NoClassDefFoundError，
        ...
        win.pack()
        // 锚点优先取 AWT 指针位置：它与 GraphicsConfiguration.bounds 同在 AWT 用户空间，
        // 而托盘事件带的坐标在 Windows 上是**设备像素**（混合缩放时根本不是同一个坐标系）。
        val pointer = runCatching { MouseInfo.getPointerInfo()?.location }.getOrNull() ?: eventAnchor
        val ge = GraphicsEnvironment.getLocalGraphicsEnvironment()
        val screens = ge.screenDevices.map { it.defaultConfiguration.bounds }
        val at = menuLocation(
            pointer, win.width, win.height, screens, ge.defaultScreenDevice.defaultConfiguration.bounds
        )
        win.setLocation(at.x, at.y)
        win.isVisible = true
        menuWindow = win
        ...
        DesktopLog.mark(
            "托盘菜单已弹出（${win.width}×${win.height} @${at.x},${at.y}，锚点 @${pointer.x},${pointer.y}）：" +
                rendered.joinToString(" / ")
        )
    }

    /**
     * 托盘菜单该落在哪 —— **纯函数**，好让 `--smoke` 喂一串多显示器矩形直接验；
     * 本机只有一块屏，真实症状（菜单弹到主屏右下角）在这里根本复现不出来。
     *
     * 坐标口径：入参 [anchor] 与 [screens] 必须同在一个坐标系里，返回的点要喂 `Window.setLocation`，
     * 而 AWT 的 `GraphicsConfiguration.bounds` 就是那个坐标系（**用户空间**）。
     * ⚠ 症状的根因就在这两个坐标系不一致：托盘事件给的 `MouseEvent.xOnScreen/yOnScreen` 是**设备像素**，
     * 与用户空间差一个缩放因子 ⇒ 多屏/混合缩放下那个点落不进任何一块屏的矩形，
     * 而旧实现"落不进就回退主屏"再被夹取 ⇒ **菜单跑到主屏右下角**。
     * 所以这里两条都改了：① 调用方改用 AWT 指针位置当锚点（已在用户空间）；
     * ② 真落不进任何一块屏时，取**离锚点最近的那块**并夹在它里面，而不是无条件回退主屏。
     *
     * [fallback] 只在 [screens] 为空时用（理论上不会发生）。
     */
    internal fun menuLocation(
        anchor: Point,
        menuWidth: Int,
        menuHeight: Int,
        ...
        val device = screens.firstOrNull { it.contains(anchor) }
            ?: screens.minByOrNull { distanceSq(it, anchor) }
            ?: fallback
        // 上界用 coerceAtLeast 兜住：菜单比屏还宽/高时（窄屏、超大菜单）不能让上界小于下界，
        // 否则 coerceIn 会抛 IllegalArgumentException，菜单直接弹不出来。
        val maxX = (device.x + device.width - menuWidth).coerceAtLeast(device.x)
        val maxY = (device.y + device.height - menuHeight).coerceAtLeast(device.y)
        return Point(anchor.x.coerceIn(device.x, maxX), anchor.y.coerceIn(device.y, maxY))
    }

    /** 点到矩形的最短距离的平方（0＝已经在里面）。坐标系不统一时也没有意义，见 [menuLocation] */
    private fun distanceSq(r: Rectangle, p: Point): Long {
        val dx = when {
            p.x < r.x -> (r.x - p.x).toLong()
            p.x > r.x + r.width -> (p.x - r.x - r.width).toLong()
            else -> 0L
        }
        val dy = when {
            p.y < r.y -> (r.y - p.y).toLong()
            p.y > r.y + r.height -> (p.y - r.y - r.height).toLong()
            else -> 0L
        }
        return dx * dx + dy * dy
    }
...
/**
 * 开机自启：写 `HKCU\...\CurrentVersion\Run`，指向当前启动器 exe。
