...
object ModelCatalog {
     ...
     * 按 Base URL 推断服务商关键词。
     *
     * ⚠️ 只在**传进来的那张表**里找（2026-09-15 第 14 轮加固）：早先的实现取「对话表 + 生图表」的并集，
     * 于是只在生图表里的供应商（硅基流动）会被 `chatPresets` 认出来、却查不到对话清单，
     * 取 `chatByKeyword[key]!!` 直接抛 NPE 炸掉设置页（就是 P0-3 那条必崩）。
    ...
    private val speechByKeyword: Map<String, List<String>> = mapOf(
        ...
        "dashscope" to listOf(
            ...
            // 见 `docs/鲸鱼-TTS-v2-调研与设计.md` §九的旧记录，那条说的是 Qwen-TTS 不是 Qwen-Audio-TTS）。
     ...
     * 返回 null 表示无需修正。
     */
    fun healSpeechSelection(baseUrl: String, model: String, voice: String): Pair<String, String>? {
        ...
        when (kw) {
            ...
            "volcengine" -> {
                ...
                if (m.isEmpty()) set(volcResourceIdForVoice(v), v)
                if (m.isNotEmpty() && !m.startsWith("seed-")) set(volcResourceIdForVoice(v), v)
                if (v.isEmpty() || !isVolcVoiceId(v)) {
     ...
     */
    private fun isVolcVoiceId(voice: String, cloneIds: Collection<String> = emptyList()): Boolean {
        val v = voice.trim()
    ...
    fun volcResourceIdForVoice(voice: String, cloneIds: Collection<String> = emptyList()): String {
        ...
        return when {
            ...
            v.isNotEmpty() && cloneIds.any { it.trim() == v } -> "seed-icl-2.0"
            ...
            // 认不出来就按 2.0 试（新版控制台的音色库基本都是 2.0 系列）
            else -> "seed-tts-2.0"
        }
    }

    /**
     * 点选「声音类型」胶囊时，「语音合成模型」那一栏该变成什么（第 69 轮）。
     *
