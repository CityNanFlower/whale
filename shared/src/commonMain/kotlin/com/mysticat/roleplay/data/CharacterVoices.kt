...
object CharacterVoices {
    ...
    fun effective(global: AiSettings, card: CharacterCard?): AiSettings {
        ...
        val isVolc = ProviderProfiles.speechProtocol(url) == ProviderProfiles.SpeechProtocol.VOLC
        val useMix = mix.size >= 2 && isVolc
        // 火山：Resource-Id（「语音模型」栏）**必须与音色家族匹配**，而卡上那栏可能还留着编辑器默认值
        // `seed-tts-2.0`（音色却选了 1.0 官方音色或复刻音色）——原样带进合成必然报
        // `resource ID is mismatched with speaker`。聊天链路只走这里、不过设置页那道自愈，
        // 所以档位要在这里按**本次真正要发的音色**重算（与编辑器选音色用的是同一份 `volcModelForKind`）。
        val cloneIds = global.ttsCloneVoices.map { it.id }
        val model = if (!isVolc) {
            v.model
        } else {
            ModelCatalog.volcModelForKind(
                when {
                    useMix -> VoiceKind.MIX
                    isCloneVoice(v.voice, v.model, cloneIds) -> VoiceKind.CLONE
                    else -> VoiceKind.SINGLE
                },
                v.voice, v.model, mix, cloneIds
            )
        }
        return global.copy(
