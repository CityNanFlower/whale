...
/**
 * 账号级世界书文件（`accounts/<id>/worldbooks/<bookId>.json`）的装载形状。
 * `bookId` 即文件名（App 生成的 UUID，不用书名——书名可改、可重名，做不了主键）。
 * 备份包里也用这个形状随行（[com.mysticat.roleplay.data.Backup] 的 `worldbooks` 节点）。
 */
@Serializable
data class WorldBookFile(
    val id: String,
    val book: WorldBook,
)
...
data class WorldBookEntry(
     ...
     *
     * **新条目默认「按深度插入」**（插进消息数组、贴着生成点）：对话实测发现"关键词命中却无效"
     * 的主因是位置——before/after_char 注进系统提示词的长上下文里会被稀释，同一资料按深度注入
     * 才真正影响当轮回复。导入不经过这个默认值（解析器显式给位，缺失回退 before_char）。
     */
