...
data class AiSettings(
    ...
    // ---- 生图专用 Key（2026-09-15 重构后降级为"兼容读取"）----
    /**
    ...
    /**
     * 是否已为**库里已有的卡**补过一次分类（自动登记是后来才有的，老卡当时没登记）。
    ...
    // ---- 合规：年龄与自愿声明（「用户 › 关于鲸鱼」那一页）----
    /**
     * 用户是否已勾选「我已满 18 周岁，自愿使用本应用」。
     *
     * 按账号存（settings.json 随账号走）。它只记"这份声明点过没有"，**不是门禁**：
     * 勾选与否都不影响任何功能可用性，导入时也只是少弹一次确认（见 `Compliance.IMPORT_CONFIRM_HINT`）。
     */
    ...
    /** 生图供应商实际使用的 Key：兼容读取生图专用 Key，其余按 [keyFor] 走共享规则 */
    fun imageKey(): String {
...
data class CloneVoice(
    ...
    /**
     * 官方给的**试听片段**地址（`demo_audio`）与拿到它的时间。
     *
     * 为什么值得存：后付费音色**首次「语音合成」才算转正收费**，而这段 URL 是免费的
     * ⇒ 想先听像不像，走它（[VoiceCloneService.fetchDemo]），不必先付一次合成费。
     * ⚠ 官方口径**1 小时有效**，所以连时间一起存 —— 过期就不放它了（[CloneVoice.demoFresh]）。
     */
    val demoAudio: String = "",
    val demoAt: Long = 0L
) {
    /** 试听片段还能用吗（官方 1 小时有效期，留 5 分钟余量） */
    fun demoFresh(now: Long = System.currentTimeMillis()): Boolean =
        demoAudio.isNotBlank() && demoAt > 0L && now - demoAt < 55 * 60 * 1000L
}
...
data class CharacterCard(
    ...
    //
    // ⚠ **老卡不反向拆**（定稿）：库里已存的卡 persona 是拼好的文本，正则拆会把用户手改过的拆坏。
     ...
     * 它是玩法形态唯一的"对错来源"——没有它，模型会为了顺着用户而放宽规则。
     */
    ...
    /**
     * 世界书：关键词触发的背景资料，兼容 SillyTavern 的 `character_book`。
     *
    ...
    /** 是不是工具形态卡——装配模式、编辑器字段裁剪、聊天页开关全部由它派生 */
    fun isTool(): Boolean = formTag == "tool"

    /** 是不是玩法形态卡——同上，装配模式与编辑器字段都按它裁 */
    fun isPlay(): Boolean = formTag == "play"

    /**
     * 这张卡吃到了 v2 分层字段没有（装配路径的判据）。
     *
...
data class Conversation(
    ...
    val backgroundUri: String? = null,
    val backgroundUriDesktop: String? = null,
    // 会话级背景暗化强度：0f~1f，压在背景图上两层黑色 scrim 的透明度系数（1f = 历史写死的默认档）。
    // null = 没动过 = 用默认；只动 scrim 不动图，换背景图后强度照常生效
    val bgDim: Float? = null,
    // 会话级背景音乐配置：null = 没设置过 = 跟随全局默认
 ...
 *
 * 为什么要专门有它（桌面端事故）：全项目过去一律写 `startsWith("/")` 当这个判据，
...
/**
 * 会话级背景音乐配置（口径：背景音乐**只在会话中生效**、一个会话一份配置）。
 *
 ...
 * - [volume]：**已退役，不再参与解析**——音量全 App 只有一个数（`AiSettings.bgmVolume`），
