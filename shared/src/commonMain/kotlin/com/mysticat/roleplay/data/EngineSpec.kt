...
data class EngineSpec(
    ...
    /**
     * 前两个字段的叫法。工具形态下它们是**风格参考**（人格降级为风格滤镜，见台账 §5.2）——
    ...
    /**
     * 灵感创作页「形态标签」下面那行说明（第 75 轮）：选了这一档会生成出什么。
     ...
     * 段名说错，模型会把"排除名单"当成"我们的关系"来对待（报告 §五 第 6 条）。
     */
     ...
     * 成品要稳，工具默认比陪伴低；用户自己调过就一律听用户的（台账 §5.5 第 4 条）。
     *
     * `null` ＝ **不接管**。玩法型故意留 null：猜谜/推理恰恰需要探索，压到 0.3 会把候选
     * 收敛到最显著的几个（二十问"逐个报人名"的第一号根因，报告 §四）。
     */
     ...
     * 但得**说出来**该配什么，否则"参数装反"的坑只是从默认值挪到了用户手上（报告 §五 第 3 条）。
     */
...
object Engines {
    ...
    /**
     * 工具（E4）：交互契约不同 —— 用户消息是**待处理素材**、输出是**可直接拿走的成品**。
    ...
    /**
     * 玩法（E4 拆档，2026-09-23）：陪用户玩**一局**有规则的玩法。
     *
     * 与 [TOOL] 同属"不扮演虚构角色"那一侧，但评判标准完全不同：工具问"成品能不能直接用"，
     * 玩法问"规则守得住、进度记得住、这一局好不好玩"（报告 §三）。三条派生差异：
    ...
    /**
     * 形态名的**可空**版本，给"徽标"这类展示用（第 80 轮发现页的精选卡 tile）：
