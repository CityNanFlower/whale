 ...
 *
 * 从 `VersionSecurityScreen` 里抽出来（台账 8）：**托盘菜单也要能查更新**，而托盘住在 desktopApp、
 * 拿不到那个 Composable 里的私有实现。抽成不依赖 UI 的一层后，两个入口（版本与安全页 / 托盘菜单）
...
data class UpdateInfo(
    ...
    val sha256: String = "",
    /** 可选：安装程序形态用的安装器地址（桌面 M13；缺省时退回 zip 链路） */
...
object UpdateChecker {
    ...
    private fun fetchOne(url: String, token: String, nodeKey: String?): UpdateInfo? {
        ...
        http.newCall(builder.build()).execute().use { resp ->
            ...
            // M5：两个平台共用同一份 version.json，桌面读 `windows` 节点（Android 仍是顶层字段）
