...
/**
 * 全量备份 / 恢复：角色卡 + 会话记录 + 账号级世界书 + 被引用的图片（formatVersion ≥ 2）。
 * 出于安全考虑，API Key 不进备份文件。
 *
 * 图片以 base64 内嵌在 assets 里（键 = 导出时的本地绝对路径），恢复时重新落盘并改写引用，
 * 这样换机恢复后头像 / 背景不再是断链；旧格式（无 assets）照常可导入。
 * 世界书（升账号级）在 `worldbooks` 节点随行；更老的备份没有这一节 * 卡里内嵌的书正文会在恢复后的读取链路上自动抽成账号级书（`Repository.resolveWorldBook` 的迁移路）。
 */
object Backup {
    ...
    data class BackupBundle(
        ...
        /** 账号级世界书：书实体随备份走，卡里只有引用；老备份无此节（靠卡内嵌自愈） */
        val worldbooks: List<WorldBookFile> = emptyList(),
        /** 本地图片内嵌：键 = 原文件绝对路径，值 = base64(文件内容) */
        val assets: Map<String, String> = emptyMap()
    )

    data class RestoreResult(val characters: Int, val conversations: Int, val images: Int, val books: Int = 0)

    /**
     * 备份要内嵌的本机图片路径：头像 / 背景（**含分端的桌面端那份**）/ **消息里的图**。
     *
     * 抽成公开纯函数是为了让桌面自检能直接验它：判据曾是 Android 专属的 `startsWith("/")`，
    ...
    fun exportJson(): String {
        ...
        return json.encodeToString(
            ...
            BackupBundle(
                characters = chars,
                conversations = convs,
                worldbooks = Repository.listWorldBooks(),
                assets = assets
            )
        )
    }
    ...
    fun importJson(text: String): RestoreResult {
        ...
        // 清空当前数据（deleteCharacter 内部已经删掉该角色的全部会话，
        ...
        // 世界书也按"重建"语义先清后建（与角色/会话同一口径）
        Repository.clearWorldBooks()

        val restored = remapBundleRefs(bundle, remap)
        restored.worldbooks.forEach { Repository.saveWorldBook(it.id, it.book) }
        restored.characters.forEach { Repository.saveCharacter(it) }
        restored.conversations.forEach { Repository.saveConversation(it) }
        return RestoreResult(bundle.characters.size, bundle.conversations.size, remap.size, bundle.worldbooks.size)
    }
}
