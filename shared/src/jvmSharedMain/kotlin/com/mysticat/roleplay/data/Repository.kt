 ...
 *   accounts/<id>/{settings.json, characters/, conversations/, user_settings.json, images/}（每账号数据）
 */
object Repository {
    ...
    fun listCharacters(): List<CharacterCard> {
        ...
        return byId.values.filterNot { it.id in pendingCharDeletes }.sortedByDescending { it.createdAt }
    }
    ...
    fun getCharacter(id: String): CharacterCard? {
        ...
        charCache[id]?.let { return it }
        val f = File(charactersDir(), "$id.json")
        return if (f.exists()) decodeFile(f, CharacterCard.serializer())?.also { charCache[id] = it } else null
    }
    ...
    fun saveCharacter(c: CharacterCard, onFailure: ((Throwable) -> Unit)? = null) {
        ...
        enqueueWrite("角色 ${c.id}", onFailure) {
            writeAtomically(f, json.encodeToString(CharacterCard.serializer(), c))
            if (prev != null) {
                reclaimImages(
                    listOf(
