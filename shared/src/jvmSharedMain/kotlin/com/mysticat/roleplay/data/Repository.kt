 ...
 * 上**恒假** ⇒ 本集合恒为空 ⇒ 全部图片都被判成孤儿被删（修过的桌面端事故，见 [isLocalFilePath]）。
 */
...
object Repository {
    ...
    /**
     * settings 内存缓存（2026-09-15 第 2 步）。
     *
    ...
    fun loadSettings(): AiSettings {
        ...
        // 明文自愈（安全审计）：磁盘上还留着明文 Key（当年加密不可用时存的，或从未重存过），
     ...
     * 代价是写失败时缓存里是"没落盘的那份"——与聊天会话写路径的处理一致。
     *
    ...
    fun saveSettings(s: AiSettings, onDone: ((Throwable?) -> Unit)? = null) {
        ...
        cachedSettingsFile = f
        cachedSettings = s
        // 背景音乐：挂在这个**唯一落库口**上，界面就不必在每个改设置的地方手抄一次
    ...
    /** 用户是否已勾选「我已满 18 周岁，自愿使用本应用」（按账号存，见 `AiSettings.complianceAccepted`） */
    fun complianceAccepted(): Boolean = loadSettings().complianceAccepted

    /**
     * 记下这份年龄与自愿声明。值没变就不写盘——勾选框与导入确认都走这里，
     * 重复点不该每次都动一遍 `settings.json`（那份写盘还要逐条加密 Key）。
     */
    fun setComplianceAccepted(accepted: Boolean) {
        val s = loadSettings()
        if (s.complianceAccepted == accepted) return
        saveSettings(s.copy(complianceAccepted = accepted))
    }
     ...
     *    「清除缓存」不该动它。
     *
     * 目录里是**多首曲目**（音乐库，清单落 `bgm_library.json`）；「只留一份」的旧规矩
     ...
     * 自检**不要自己拼这个路径** —— 这个坑就是这么暴露的：冷数据根（沙箱/全新目录）下没有当前账号，
    ...
    // ---- 音乐库：多曲目 + 应用内显示名（重命名**不改**源文件名，用户口径）----
    ...
    /**
     * 给一张卡挂上 / 摘掉一本书：宝库编辑区逐卡开关的唯一写入口。
     * `bookId = null` ＝ 解除引用。走 [saveCharacter] 的常规落盘链路（缓存 / 图片回收都在同一条路上）；
     * 内嵌正文清空——落盘时 [saveCharacter] 本来就会剥，这里先剥掉免得中间态带两份。
     */
    fun setCharacterWorldBook(characterId: String, bookId: String?, onFailure: ((Throwable) -> Unit)? = null) {
        val c = getCharacter(characterId) ?: return
        val fixed = if (bookId != null && validBookId(bookId)) c.copy(worldBookId = bookId, worldBook = null)
        else c.copy(worldBookId = null, worldBook = null)
        saveCharacter(fixed, onFailure)
    }

    /**
     * 存角色：**不阻塞调用线程**，加密无关、但角色 JSON 可能带着长人设与开场白，
     * 而保存动作发生在编辑页返回的路径上。
     *
    ...
    /**
     * 删角色：连它的头像、角色背景、以及**它全部会话**的背景图与聊天图一起回收。
    ...
    /**
     * 会话文件扫描缓存（2026-09-15；实现挪进 [DirScanCache]，与角色共用一份）。
     *
     ...
     *
     * 整个动作放到写入线程上（它可能要遍历全部数据算引用集合、再删掉一批图）。
    ...
    data class CacheBreakdown(
        ...
        /** `tts/` 语音朗读缓存（一键清理**保留**它，否则同句朗读要重新付费合成） */
        val ttsBytes: Long,
    ...
    /**
     * 一键清理＝清缓存，**`tts/` 整棵保留**（删它不影响正确性，但同句重听要重新付费合成）。
     *
     * ⚠ **2026-09-21 改成白名单**（用户实测：桌面端「清除缓存」把角色卡也清了，数据丢失）：
     ...
     *
     * 为什么是白名单而不是"除 tts 全删"：见 [clearCachesIn] 的说明。
     * 注：`tts/` 不在表里＝按既定口径**保留**；`backups/` 也不在＝那是用户的导出备份，不能当缓存清。
     */
     ...
     * 只有自检拦得住；它确实算错过：判据是 Android 专属的 `startsWith("/")`）。
     */
     ...
     * 删除账号。
     *
     * 原来只删 `profiles.json` 里的记录与 current 指针，
