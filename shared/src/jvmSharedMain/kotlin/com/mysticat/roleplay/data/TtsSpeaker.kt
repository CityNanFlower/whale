 ...
 *
 * M3 跨平台拆分：本类只剩**编排**（切句、攒句、队列、回退）；"怎么发声"走 [TtsEngine]
...
class TtsSpeaker(
    ...
    /**
     * 朗读状态位：**所有**赋值都走这里，因为它是"背景音乐让路"的唯一触发点（第 59 轮）。
     *
    ...
    /**
     * 朗读代次（第 69 轮修）：**每次 [stop] 与每一次新的朗读都 +1**，只有"当前代次"的循环能继续跑。
     *
    ...
    suspend fun speakBuiltin(text: String, synth: Synth) {
        ...
        for ((index, piece) in pieces.withIndex()) {
            ...
            // 任何一次抖动都让"语音明明在放"的同时弹出"合成失败"（第 57 轮，用户报的通病）
            var file: File? = null
     ...
     *
     * 用 [kotlinx.coroutines.CompletableDeferred] 而不是 `CancellableContinuation`（第 28 轮改）：
     ...
     *
     * ⚠️ **`p.start()` 必须离开主线程**（第 28 轮踩出来的第二条）：它要解码整段音频
