 ...
 * （见 androidMain 的实现与 App.kt 的装配），桌面 M4 接 javax.sound 与系统语音。
 ...
 *
 * 第 59 轮起也承担**背景音乐**：[loop] 循环播放、[start] 带初始音量、[setVolume] 播放中调音量
...
object Voice {
     ...
     *
     * ⚠ **声明出去就必须真能放**（第 60 轮用户要求支持 m4a 之后这条更要紧：多写一个扩展名，
     * 就是把上面那种静默故障重新引进来）。两端的取值分别是：
     ...
     *   m4a/aac、flac、ogg 靠第 60 轮加的 `com.tianscar.javasound` 三个 SPI。
