 ...
 *
 * M3 跨平台拆分：本文件只留**共享的对话框编排与文件名规则**；渲染落盘（buildCardImageFile）、
 * 系统分享、剪贴板、JSON 保存位置全部走 [PlatformUi]（Android 实现在 androidMain，桌面 M4 补齐）。
 */
...
/**
 * PNG 角色卡的字节（台账 11）：头像图作载体——已是 PNG 的头像直接嵌数据（原图零损耗），
