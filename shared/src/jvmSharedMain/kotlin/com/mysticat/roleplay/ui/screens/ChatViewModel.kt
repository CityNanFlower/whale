...
import com.mysticat.roleplay.data.suggestReplies
...
class ChatViewModel(
    ...
    var showSessionPanel by mutableStateOf(false)
    ...
    fun openInspirations() {
        // 工具形态整个禁掉：灵感回答的是"戏里我该说什么"，而工具里用户要发的是任务素材
        // 入口也不渲染，这里再兜一道，免得别处的调用漏进来。
        if (!engine.inspiration) return
    ...
    /**
     * 顶栏喇叭开关「新回复自动朗读」的**显示值**（真值在 [AiSettings.ttsAutoRead]）。
     *
     * 与 [voiceTapToTalk] 同理：设置是全局的、别的页面（设置页的「语音」分段）也能改，
     * 而本 ViewModel 活过那一趟 ⇒ 不能在构造时读死。不重算的症状是**开关与真实设置相反**：
     * 用户在设置页打开自动朗读，回聊天页看到的还是「关」，点一下以为在开、写回去的其实是「关」。
     *
     * 朗读与否本身一直是对的——发送路径现读设置（见 `send` 里的 `settings.ttsAutoRead`），
     * 错的只有这颗图标的显示，但"界面撒谎"照样会让人以为功能坏了。
     */
    fun refreshAutoRead() {
        ttsAutoRead = Repository.loadSettings().ttsAutoRead
    }
     ...
     *
     * `init` 里的同步读已搬到后台：`listConversations` 要在界面线程上把账号目录下
    ...
    init {
        ...
        viewModelScope.launch {
            ...
            refreshVoiceMode()
            refreshAutoRead()
            // 背景音乐只在会话中生效（架构口径）：进聊天页按本会话的配置起播
            // （Conversation.bgm 为 null＝从没配过 ⇒ 跟随全局默认）。
            // 句柄要留着：桌面三栏切会话时"新的先进、旧的后离开"，离开不带句柄会把新会话的音乐停掉
    ...
    private fun updateConversation(
        ...
        // 落盘已不在主线程（Repository 走串行写入通道），失败回调被转回主线程再提示
     ...
     * 落盘并同步内存态。
     *
     * 落盘不再阻塞主线程，改为交给 `Repository` 的串行写入通道；
    ...
    fun send() {
        ...
        val withUser = conv.copy(
            ...
            // 工具形态下首条消息是**待处理的长素材**，直接截前 18 字会截在句子中间，
            // 所以取第一行、放宽到 24 字 —— 会话列表里要能认出"这是哪一单"。
     ...
     *
     * **节奏与口径按形态走**：陪伴 / 多线 10 条一次（关系变化慢，够用且省后台请求）；
    ...
    private fun maybeUpdateMemory(convId: String, messages: List<ChatMessage>) {
        ...
        val sys = if (play) {
            ...
            "你是记忆管理器。把【现有记忆】与【新对话】合并为更新后的角色记忆：\n" +
                ...
                "4. 记忆里只收对话中双方明确说出或确认的内容；角色自述的来历、身世、往事若没有对话佐证，" +
                "整行不写——也不要写「用户已指出/已纠错」这类没有发生过的事；\n" +
                "5. 出现新的专有名词（人名/地名/组织）而设定资料里没有对应条目时，记成「待确认：X」；\n" +
                "6. 输出保持正常要点行，不要用「［…］」批注体。\n" +
        ...
        viewModelScope.launch {
            try {
                ...
                // 后台任务：固定关闭思考，别让深度推理拖慢/变贵（用户要求）
                // 失败/空回复**当场重试一次**：实测一次失败＝旧记忆续命到下个合并周期（最长十轮），
                // 实验里"守山人/续蜡人"的整条虚构支线就是这么冻出来的。
                var result = ""
                repeat(2) {
                    if (result.isBlank()) {
                        result = runCatching {
     ...
     * 写入发生在协程（IO 线程的 onCall 回调），读取发生在主线程 onCleared，
    ...
    override fun onCleared() {
        ...
        // 背景音乐只在会话中生效（架构口径）：离开聊天页（含桌面切走第三栏的聊天窗）就停。
        // 交回自己的句柄：桌面切会话时本方法可能**晚于**新会话的 enterSession 才跑
     ...
     *
     * 初值留空、由 [refreshAutoRead] 在进会话与进页面时读，见那里的说明（它是"别处也能改"的一项）。
     */
    ...
    private fun requestReply(history: List<ChatMessage>, variantOf: Int = -1) {
        ...
        viewModelScope.launch {
            ...
            // 工具形态关掉受控揭示：成品要立刻能复制走，不该被 16ms/拍往后推。
            ...
            try {
                ...
                // （它们被整个隐藏），拿去做摘要既没意义又要花钱。
                // 玩法形态也跳过：一局要延续的是**进度**而不是"更早剧情的摘要"，
                ...
                // 四种形态都允许自动朗读（`engine.autoTts`）：朗读是输出通道，与形态无关。
                ...
                if (trimmed.isNotBlank()) {
                    ...
                    if (variantOf >= 0) {
                        ...
                        // ⚠️ **必须以 messagesBeforeReply 为底，不能往 `stored.messages` 上追加**（2026-09-15 查到）：
    ...
    fun setBackground(uri: String?, target: BgTarget = bgMenuTarget) {
        ...
        // 换掉/移除旧背景后回收它（引用检查在 Repository 里做，角色默认背景不会被误删）
        Repository.reclaimImages(listOf(old))
    }

    /**
     * 会话级背景暗化强度：压在背景图上两层 scrim 的透明度系数。
     * 滑杆拖动中只更新界面草稿，松手（onValueChangeFinished）才落盘——persist 会重写整份会话 JSON，
     * 逐帧写盘不值得。传 null = 回到默认档。
     */
    fun setBgDim(v: Float?) {
        val conv = conversation ?: return
        persist(conv.copy(bgDim = v))
    }

    /**
     * 把 [target] 端当前生效的背景复制到另一端：两张图都在本机，只是改字段。
     * 走 [setBackground] 的回收口径——另一端被替换掉的旧图若无引用会被清理。
     * 生效值可能是继承自角色卡的那张，复制后它在本会话内成为另一端自己的设置。
     */
    fun copyBackgroundToOtherEnd(target: BgTarget = bgMenuTarget) {
        val conv = conversation ?: return
        val uri = effectiveBackground(character, conv, target) ?: return
        setBackground(uri, if (target == BgTarget.Desktop) BgTarget.Phone else BgTarget.Desktop)
    }
     ...
     *
     * 角色卡的 `post_history_instructions`排在最前：它是**长期**要求（作者设定的始终要遵守的规则），
    ...
    private fun tailHint(style: NarrativeStyle, card: CharacterCard?): String = buildString {
        ...
        // 工具形态：同一套机制（贴着生成点）、**不同数据源** —— 7 轴已隐藏，
        // 改由「输出格式」的可核对硬要求产出。实测有效的正是这种"可直接核对"的话
        // （"只给 10 个编号名字、不要别的字"），而不是抽象的文学风格。
        ...
        if (engine.promptMode == PromptMode.TOOL) {
            ...
            // 玩法形态：这里**必须换掉**叙事风格的硬性要求 —— 它会要求"本回合写满 250~500 字"，
            ...
            // 常驻兜底（2026-09-26 实验定稿）：这两条必须贴着生成点才生效——同类话放系统提示词
            // 会被长上下文稀释。防重复令是全供应商兜底（没发采样惩罚的家也压得住复读）；
            // 视角 enforce 用卡名指代、人称中立，每条都重申一遍。
            if (card != null) {
                if (isNotEmpty()) append('\n')
 ...
 * 跨平台拆分后语音能力走 [Voice]/[Platform] 注入，ViewModel 不再需要 Context，
