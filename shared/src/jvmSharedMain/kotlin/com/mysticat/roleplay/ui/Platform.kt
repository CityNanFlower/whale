...
interface PlatformUi {
    ...
    /** 返回键拦截（Android=系统返回键；桌面 M4=Esc，由窗口壳转成同一回调） */
    @Composable
     ...
     *
     * ⚠ **本机实测（第 27 轮）：挑与不挑，中文正文逐像素一致**（本机 fallback 本来就是雅黑），
    ...
    /**
     * 「左右可拖」手柄的鼠标光标（第 58 轮，用户反馈：拖侧边栏时指针要变成左右箭头，
     * 否则用户不知道那里能拖）。默认实现**原样返回**——Android 没有鼠标指针，套上也没意义。
     *
    ...
    /**
     * 生成 PNG 角色卡（台账 11）：头像图（无头像时用渲染的竖版长图）作载体，
     ...
     * 它那边有 logcat 与 CrashGuard 两条路，日志不上界面（分平台口径见台账 P3 `#4`）。
     */
    ...
    /**
     * 更新分流（桌面 M13 安装程序形态）：程序目录不可写（装进 Program Files）时改走
    ...
    // ────────────────────── 应用内更新（Android=ApkUpdater；桌面 M5=便携包更新器） ──────────────────────
    ...
    // ────────────────────── 桌面专属三件（1.0.2 台账 1/2/3；Android 用不到，恒 false） ──────────────────────
...
/**
 * 无参 ViewModel 的**跨平台**工厂（第 23 轮新增，桌面回归的根因）。
 *
...
/**
 * 兜底实现：宿主没注入时保证不崩。
 * M4（第 21 轮）由 object 改为 **open class**：桌面实现 [DesktopPlatformUi] 继承它，
 * 只覆盖自己真正实现得出来的那些方法，其余继续走兜底——不必为没实现的能力写一遍空壳。
 */
