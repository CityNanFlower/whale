...
object DesktopDragDrop {
    ...
    /** PNG 角色卡：图里嵌着 `chara` tEXt 数据的才算（纯图片的 PNG 仍走图片链路） */
    ...
    suspend fun handleFiles(paths: List<String>): String? = withContext(Dispatchers.IO) {
        ...
        if (cards.isNotEmpty()) {
            // 合规：还没勾过年龄声明时**不静默导入**，给一条指路（拖拽这条入口没有弹窗可挂，
            // 角色卡页那两条入口会弹确认框——两边防的是同一件事）
            if (!Repository.complianceAccepted()) {
                return@withContext "请先在「用户 › 关于鲸鱼」确认使用须知，再拖入角色卡"
            }
            var imported = 0
