 ...
 * 用户 2026-09-16 拍了这一刀（依据见 `docs/鲸鱼-TTS-v2-调研与设计.md` §6.2 / §6.3）。
 *
...
fun VoiceScreen(
    ...
    // 有未保存修改时，系统返回键也要走确认，避免一次误触丢掉刚落盘的配置
    ...
    val body: @Composable (PaddingValues) -> Unit = { padding ->
        LazyColumn(
            ...
            item {
                // 引子跟着**当前用法**说话（第 32 轮）：切成"点按"后这句话还写"按住"就自相矛盾了
                Text(
                    buildString {
                        append("聊天页输入框右侧的麦克风：")
                        append(
            ...
            // 麦克风用法（第 32 轮）：桌面固定点按，所以只在手机端给这个选择，免得桌面出现一个不起作用的开关
            if (!isDesktopLayout) {
                item {
                    Column {
            ...
            if (vm.ttsProvider == "builtin") {
                ...
                // 声音类型（第 69 轮）：单一 / 混合 / 复刻。**类型不落库**，由下面几栏的数据推出来
 ...
 *
 * 第 69 轮起：这一段**不再自带开关**——"是不是混合音色"由上面的「声音类型」胶囊决定
...
private fun MixSpeakerSection(vm: SettingsViewModel, enabled: Boolean) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
        ...
        // 第 70 轮抽出：用户反馈"角色专属音色也要能够用预设和存预设"——那边正是缺了这一段。
        MixPresetBar(
