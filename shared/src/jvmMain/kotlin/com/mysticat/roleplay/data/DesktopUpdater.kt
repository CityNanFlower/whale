...
/**
 * 桌面版更新器（对齐 Android 的 [ApkUpdater]，但桌面的"安装"是**换便携包**而不是装 APK）。
 *
...
object DesktopUpdater {
    ...
    suspend fun download(
        ...
        /** 安装器形态传 "exe"：扩展名决定 installUpdate 的分流（exe＝拉起安装器，zip＝解压替换） */
        fileExt: String = "zip"
    ...
    /**
     * 应用目录可写性（更新分流）：安装程序形态默认装进 Program Files，普通用户没有写权限     * 「下载 zip → 解压覆盖」链路失效。据此分流：可写＝便携包替换（便携 zip / 装在用户可写目录）、
