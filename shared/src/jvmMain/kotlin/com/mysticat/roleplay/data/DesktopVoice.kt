...
class DesktopAudioPlayer : AudioPlayerEngine {
     ...
     *
     * ⚠️ **绝对不要在这里 close(src)**（用 jstack 抓到的真 bug）：
    ...
    private fun toPcmIfNeeded(file: File, src: AudioInputStream): AudioInputStream? {
        ...
        try {
            ...
            // 有些 SPI 只提供**大端**目标，小端会被直接拒（实测：m4a/aac 的 jaad SPI 就是这样——
    ...
    override fun start(): File {
        ...
        pump = Thread({
            ...
            while (l.isOpen) {
                ...
                // 早期写法在这里 break，实际表现是"按住好几秒却一个字节都没录到"（实测）
                if (n < 0) break
                if (n == 0) {
                    Thread.sleep(10)
                    continue
 ...
 * 用户看到的就是**"按住还没说话、三秒就自动结束"**（实测 2.96 秒，与用户反馈一致）。
...
fun installDesktopVoice(toolsDir: File, cacheRoot: File) {
    ...
    // Java Sound 原生认 WAV/AIFF/AU，MP3 靠 mp3spi；又装了三个 SPI
