...
/**
 * 桌面端平台实现（平台替身清单见 Platform.kt）。
 *
 * 继承 [FallbackPlatformUi]（由 object 改为 open class），**只覆盖桌面实现得出来的**；
 * 还没实现的继续走兜底，兜底行为是"提示 + 返回取消/失败"，不会静默假装成功。
 * 补齐了图片卡渲染（[buildCardImageFile]）——更新器仍是待办。
 */
class DesktopPlatformUi(
    ...
    override fun rememberJsonFilePicker(onPicked: (String?) -> Unit): () -> Unit {
        ...
        return {
            scope.launch {
                // png 一并放进来：PNG 角色卡与 JSON 走同一个导入入口
    ...
    override fun rememberApkPicker(onResult: (Boolean) -> Unit): () -> Unit {
        ...
        // 这里让用户选一个 zip，校验后走与自动更新同一套 staging
        return {
            scope.launch {
                ...
                val ok = withContext(Dispatchers.IO) {
                    runCatching {
                        ...
                        // 就绪后不再 toast 让用户翻资源管理器，交给「关于鲸鱼」页的确认层
    ...
    /** PNG 角色卡：头像作载体嵌 JSON，落 `缓存目录/card-export/`，分享/保存与图片卡同链路 */
    ...
    /** 安装程序形态：程序目录不可写（Program Files）时更新改走"下载新安装器" */
    ...
    /** 应用流水日志：卡死类问题的唯一取证物，见 [DesktopLog] 的说明 */
    ...
    /**
     * 桌面的"安装"＝解压暂存 + 生成替换脚本，然后交给「关于鲸鱼」页的确认层
     * （默认"立即安装"＝拉起脚本＋退出，脚本会等进程退出、覆盖、自动重启；
     * 用户选"稍后"才落回旧的"去资源管理器里双击"口径）。
     */
    override fun installUpdate(apk: File): Boolean {
        // 安装器形态：下载到的是 jpackage 安装器 exe，拉起它走系统安装向导
     ...
     *  启动类加载器），看不到应用类路径，getResourceAsStream 恒为 null——坏了很久，
 ...
 *
 * ⚠ 本机实测：不挑时 fallback 本来就落到雅黑，**中文正文逐像素一致**——所以
