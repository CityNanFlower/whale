...
- **世界书（酒馆卡的 `character_book` 世界观词条）**：导入后按关键词生效（关键词触发、常驻条目、
  插入位置与深度都对 ST 兼容，条目与书本的未知字段原样保真）。**编辑界面与单独的 JSON 导入导出还在开发中**。
- 世界书里 `probability` 随机命中、`group` 互斥分组、递归扫描三类设置目前**只保真不生效**（字段一个字不改动，
  只是不参与判断）。
